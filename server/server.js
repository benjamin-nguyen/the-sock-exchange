...
// Endpoint to read and send socks by color
app.get('/socks/:color', async (req, res) => {
    try {
        ...
        const matchingSocks = socks.filter(sock => sock.color.toLowerCase() === color.toLowerCase());

        if (matchingSocks.length === 0) {
            // If no socks are found, return 404
            res.status(404).send(`No socks found with color: ${color}`);
        } else {
